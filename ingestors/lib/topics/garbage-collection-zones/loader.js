...
function insertZone(db, zone) {
  ...
  return Promise.resolve(null)
    ...
    .then(() => { console.log("INSERT SUCCESS") }, (err) => { console.log("INSERT FAILED: ", err); return Promise.reject(err)})
}
...
exports.load = function(zones) {
  ...
  zones.forEach(zone => {
    p = p.then(() => {
      console.log(`Inserting garbage-collection-zones feature ${zone.featureId} from resource ${zone.resourceId} from package ${zone.packageId}`)
